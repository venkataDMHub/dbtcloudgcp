...
with
    p2p_payment as (
        ...
        select
            ...
            ledgers.main_party_user_id as sender_id,
            ledgers.counter_party_user_id as recipient_id,
    ...
    p2p_request as (
        ...
        inner join
            ...
            and ledgers.main_party_user_id = details.recipient_id
    ...
    stock_trade_buy as (
        ...
        select
            ...
            array_construct(stocks.transfer_id) as stock_trade_transfer_id,
            ...
            array_construct(transfers.transfer_id) as transfers_id,
            transfers.sender_id,
        ...
        left join
            ...
            and cast(
                transfers.ledger_timestamp as date
            ) between cast(stocks.created_at as date) and (
                cast(stocks.created_at as date) + interval '{{day_interval}}'
            )
            and transfers.ledger_timestamp >= stocks.created_at
            and stocks.amount
            between transfers.ledger_amount
            * (-0.8) and transfers.ledger_amount
            * (-1.25)
...
select
    ...
    array_cat(transfers_id, stock_trade_transfer_id) as list_of_txns
from stock_trade_buy
