...
with
    ...
    pegasus_withdrawals as (
        ...
        select
            ...
            transfers.destination_currency as fee_currency,
            case
                when destination_amount >= 0 and destination_amount < 60000
                then '300'
                when destination_amount >= 60000 and destination_amount < 500000
                then '600'
                when destination_amount >= 500000
                then '1200'
            end as fixed_fee,
            destination_amount * {{ pegasus_fee_pct }} as percentage_fee,
            iff(percentage_fee >= 2006, 2006, percentage_fee) as percentage_fee_with_fee_cap,
            fixed_fee + percentage_fee_with_fee_cap as fee,
        ...
        where
            ...
            and transfers.destination_currency = 'UGX'
