...
with
    ...
    rave_deposits as (
        ...
        select
            ...
            origin_ngn_parallel_rates.rate as parallel_rate,
            case
                when transfers.hlo_updated_at < '2023-03-15' 
                then transfers.origin_amount * 0.014
                when transfers.hlo_updated_at >= '2023-03-15'
                then transfers.origin_amount * {{ rave_fee_pct }}
            end as percentage_fee,
            case
                when transfers.hlo_updated_at < '2023-03-15'
                then iff(percentage_fee >= 1000, 1000, percentage_fee) 
                when transfers.hlo_updated_at >= '2023-03-15'
                then iff(percentage_fee >= 300, 300, percentage_fee) 
            end as fee,
