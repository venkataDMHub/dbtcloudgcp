...
with
    ...
    rave_withdrawals as (
        ...
        select
            ...
            destination_ngn_parallel_rates.rate as parallel_rate,
            coalesce(
                details.TRANSACTION_DETAILS:externalProviderTransactionDetails:settled:transfer:fee::number,
                details.TRANSACTION_DETAILS:externalProviderTransactionDetails:status_payment_submitted:data:fee::number,
                details.TRANSACTION_DETAILS:externalProviderTransactionDetails:status_payment_submitted:data:data:fee::number,
                details.TRANSACTION_DETAILS:externalProviderTransactionDetails:queued_for_refund:fee::number
            ) as provider_fee,
            case
                when transfers.destination_currency = 'NGN' and transfers.hlo_updated_at < '2023-03-15'
                then '45'
                when transfers.destination_currency = 'NGN' and transfers.hlo_updated_at >= '2023-03-15'
                then '25'
                else provider_fee
        ...
        left join
            ...
            on cast(transfers.hlo_created_at as date) = destination_ngn_parallel_rates.date
        ...
        where
            ...
            and details.external_provider = 'RAVE'
...
select
    ...
    fee,
    fee_in_usd,
    case when fee_currency = 'NGN' then (fee / parallel_rate) else fee_in_usd end as fee_in_usd_parallel
from rave_withdrawals
