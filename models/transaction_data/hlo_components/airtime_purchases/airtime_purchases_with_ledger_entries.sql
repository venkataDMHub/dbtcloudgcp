...
select
    ...
    case
        when is_transfer_reversal = TRUE then concat('AIRTIME_PURCHASES', '_', 'REVERSAL')
...
from {{ ref('airtime_purchases_with_all_transfer_ids') }} as airtime_hlo


inner join {{ ref('ledger_entries') }} as ledger_entries
    on ledger_entries.journal_id = airtime_hlo.hlo_journal_id
