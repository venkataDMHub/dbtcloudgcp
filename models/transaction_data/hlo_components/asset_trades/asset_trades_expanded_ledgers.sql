...
select
    ...
    coalesce(lag(user_id) over (partition by hlo_journal_id order by amount),
        lead(user_id) over (partition by hlo_journal_id order by amount)
    ) as counter_party_user_id

from
    {{ ref('asset_trades_with_ledger_entries') }} as asset_trades_with_ledger_entries
inner join
    "CHIPPER".{{ var("core_public") }}."JOURNALS"
    on journals.id = asset_trades_with_ledger_entries.hlo_journal_id

inner join
    {{ ref('transfers_with_usd') }} as transfers_with_usd
    on asset_trades_with_ledger_entries.transfer_id = transfers_with_usd.id
