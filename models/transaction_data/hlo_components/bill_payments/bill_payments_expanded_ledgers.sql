...
from
    {{ ref('bill_payments_with_ledger_entries') }} as bill_payments_with_ledger_entries
inner join
    chipper.{{ var("core_public") }}.journals
    on journals.id = bill_payments_with_ledger_entries.hlo_journal_id

inner join
    {{ ref('transfers_with_usd') }} as transfers_with_usd
    on bill_payments_with_ledger_entries.transfer_id = transfers_with_usd.id
