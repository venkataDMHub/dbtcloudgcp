...
select
    ...
    case
        when is_transfer_reversal = TRUE then concat('BILL_PAYMENTS', '_', 'REVERSAL')
...
from {{ref('bill_payments_with_all_transfer_ids')}} as bill_payments_hlo
inner join {{ ref('ledger_entries') }} as ledger_entries
    on ledger_entries.journal_id = bill_payments_hlo.hlo_journal_id
where
