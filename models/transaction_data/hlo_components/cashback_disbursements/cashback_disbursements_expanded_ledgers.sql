...
from {{ ref('cashback_disbursements_with_ledger_entries') }} as cashback_disbursements_with_ledger_entries
inner join "CHIPPER".{{ var("core_public") }}."JOURNALS" as journals
    on cashback_disbursements_with_ledger_entries.hlo_journal_id = journals.id
inner join {{ ref('transfers_with_usd') }} as transfers_with_usd
    on cashback_disbursements_with_ledger_entries.transfer_id = transfers_with_usd.id
{% if is_incremental() %}
