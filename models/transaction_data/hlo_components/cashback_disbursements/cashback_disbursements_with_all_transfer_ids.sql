...
select
    ...
    cashbacks.cashback_transfer_id as transfer_id,
    false as is_original_transfer_reversed,
    false as is_transfer_reversal
from "CHIPPER".{{ var("core_public") }}."CASHBACKS" as cashbacks
inner join "CHIPPER".{{ var("core_public") }}."DISBURSEMENTS" as disbursements
    on cashbacks.cashback_transfer_id = disbursements.transfer_id
where 
    cashbacks.cashback_transfer_id is not null
    {% if is_incremental() %}
