...
from
    {{ ref('checkouts_with_ledger_entries') }} as checkouts_with_ledger_entries
inner join
    "CHIPPER".{{ var("core_public") }}."JOURNALS"
    on journals.id = checkouts_with_ledger_entries.hlo_journal_id

inner join
    {{ ref('transfers_with_usd') }} as transfers_with_usd
    on checkouts_with_ledger_entries.transfer_id = transfers_with_usd.id
{% if is_incremental() %}
