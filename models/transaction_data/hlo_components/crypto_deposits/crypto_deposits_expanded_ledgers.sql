...
from
    {{ ref('crypto_deposits_with_ledger_entries') }} as crypto_deposits_with_ledger_entries
inner join
    "CHIPPER".{{ var("core_public") }}."JOURNALS"
    on journals.id = crypto_deposits_with_ledger_entries.hlo_journal_id

inner join
    {{ ref('transfers_with_usd') }} as transfers_with_usd
    on crypto_deposits_with_ledger_entries.transfer_id = transfers_with_usd.id
