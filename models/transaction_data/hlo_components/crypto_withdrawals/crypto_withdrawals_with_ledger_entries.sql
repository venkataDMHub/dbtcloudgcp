...
select
    ...
    case
        when is_transfer_reversal = true then concat('CRYPTO_WITHDRAWALS', '_', 'REVERSAL')
...
from {{ ref('crypto_withdrawals_with_all_transfer_ids') }} as crypto_withdrawals

inner join {{ ref('ledger_entries') }} as ledger_entries
    on ledger_entries.journal_id = crypto_withdrawals.hlo_journal_id
...
where
    ...
    and (
        ...
        or (ledger_entries.amount > 0 and is_transfer_reversal = true)
    )
    {% if is_incremental() %}
