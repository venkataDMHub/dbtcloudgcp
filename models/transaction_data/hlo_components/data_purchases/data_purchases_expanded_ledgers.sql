...
from 
    {{ ref('data_purchases_with_ledger_entries') }} as data_purchases_with_ledger_entries
join 
    "CHIPPER".{{ var("core_public") }}."JOURNALS"
  on journals.id = data_purchases_with_ledger_entries.hlo_journal_id

join 
    {{ ref('transfers_with_usd') }} as transfers_with_usd
  on data_purchases_with_ledger_entries.transfer_id = transfers_with_usd.id
{% if is_incremental() %}
