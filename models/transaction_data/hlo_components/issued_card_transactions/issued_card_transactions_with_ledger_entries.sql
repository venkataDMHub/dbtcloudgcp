...
select
    ...
    case
        when
            is_transfer_reversal = true then concat('ISSUED_CARD_TRANSACTIONS', '_', 'REVERSAL')
...
from
    {{ ref('issued_card_transactions_with_all_transfer_ids') }} as issued_card_transactions
inner join
    {{ ref('ledger_entries') }} as ledger_entries on
        ledger_entries.journal_id = issued_card_transactions.hlo_journal_id
where
    ...
    and (
        (
            ...
            and hlo_type = 'FUNDING'
        )

        or (
            ledger_entries.amount > 0
            and is_transfer_reversal = false
            and hlo_type = 'WITHDRAWAL'
        )
