...
from
    {{ ref('orders_with_ledger_entries') }} as orders_with_ledger_entries
inner join
    "CHIPPER".{{ var("core_public") }}."JOURNALS"
    on journals.id = orders_with_ledger_entries.hlo_journal_id

inner join
    {{ ref('transfers_with_usd') }} as transfers_with_usd
    on orders_with_ledger_entries.transfer_id = transfers_with_usd.id
{% if is_incremental() %}
