...
select
    ...
    case when is_transfer_reversal = true then 'REFERRAL_BONUS_REVERSAL'
        else concat('REFERRAL_BONUS_', hlo_status)
...
from {{ ref('referral_bonus_disbursements_with_all_transfer_ids') }} as referral_disbursements_hlo
inner join {{ ref('ledger_entries') }} as ledger_entries
    on referral_disbursements_hlo.hlo_journal_id = ledger_entries.journal_id
where
