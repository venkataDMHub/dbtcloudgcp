...
from
    {{ ref('requests_with_ledger_entries') }} as requests_with_ledger_entries
inner join
    "CHIPPER".{{ var("core_public") }}."JOURNALS"
    on journals.id = requests_with_ledger_entries.hlo_journal_id

inner join
    {{ ref('transfers_with_usd') }} as transfers_with_usd
    on requests_with_ledger_entries.transfer_id = transfers_with_usd.id
