...
from
    {{ ref('stock_trades_with_ledger_entries') }} as stock_trades_with_ledger_entries
inner join
    "CHIPPER".{{ var("core_public") }}."JOURNALS"
    on journals.id = stock_trades_with_ledger_entries.hlo_journal_id

inner join
    {{ ref('transfers_with_usd') }} as transfers_with_usd
    on stock_trades_with_ledger_entries.transfer_id = transfers_with_usd.id
