...
from
    {{ ref('withdrawals_with_ledger_entries') }} as withdrawals_with_ledger_entries
inner join
    "CHIPPER".{{ var("core_public") }}."JOURNALS"
    on journals.id = withdrawals_with_ledger_entries.hlo_journal_id

inner join
    {{ ref('transfers_with_usd') }} as transfers_with_usd
    on withdrawals_with_ledger_entries.transfer_id = transfers_with_usd.id

{% if is_incremental() %}
    -- this filter will only be applied on an incremental run
    where ledger_timestamp >= (select max(ledger_timestamp) from {{ this }})
{% endif %}
