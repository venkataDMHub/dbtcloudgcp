...
select
    ...
    case
        when is_transfer_reversal = true and is_s2nc = false then concat('WITHDRAWALS', '_', 'REVERSAL')
        when is_transfer_reversal = true and is_s2nc = true then concat('S2NC', '_', 'REVERSAL')
        when is_transfer_reversal = false and is_s2nc = true then concat('S2NC', '_', hlo_status)
...
from
    {{ ref('withdrawals_with_all_transfer_ids') }} as withdrawals
inner join
    {{ ref('ledger_entries') }} as ledger_entries on
        ledger_entries.journal_id = withdrawals.hlo_journal_id
where
